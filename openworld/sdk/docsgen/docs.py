...
from openworld.sdk.docsgen.models import Module, Class, Method, Alias, Attribute
...
class ConstructorComponent:
    ...
    def __str__(self):
        constructor_docs = [
                               ...
                               self.classname + constant.OPENING_PARANTHESIS
                           ] + [
...
class MethodComponent:
    ...
    def __str__(self):
        ...
        for argument in self.method.arguments:
            argument_count += 1
            method_params.add_row([
                argument.name,
                get_datatype_reference(argument.type_hint),
        ...
        method_header = "\n".join([
                                      ...
                                      constant.CLOSING_PARANTHESIS,
                                      constant.MARKDOWN_CODE_BLOCK_SUFFIX
                                  ]) if self.method.arguments else "\n".join([
            constant.MARKDOWN_CODE_BLOCK_PREFIX,
...
class ClassComponent:
    ...
    def __init__(self, class_: Class, parent_breadcrumb: Breadcrumbs):
        ...
        self.breadcrumbs = Breadcrumbs(
            current=f"[{self.name}](./{self.file})",
            previous=[parent_breadcrumb.breadcrumbs]
        )
        ...
        self.constructor = ConstructorComponent(
            classname=self.name,
            params=params
        )
    ...
    def __str__(self):
        ...
        methods_components: list[MethodComponent] = [
            MethodComponent(method) for method in self.class_.methods
        ]

        methods = ("\n" + constant.NEW_LINE + "\n").join([
            str(method_component) for method_component in methods_components if
            method_component.method.name != constant.__INIT__
        ])

        for attribute in self.class_.attributes:
            attributes_toc.add_row([
                attribute.name,
                get_datatype_reference(attribute.type_hint),
        ...
        return "\n".join([
            ...
            " > ".join(
                get_datatype_reference(base) if base != self.class_.name else base for base in self.class_.bases),
        ])
...
class ModuleComponent:
    ...
    def __init__(self, module, parent_breadcrumb: Breadcrumbs):
        self.module = module
        self.breadcrumbs = Breadcrumbs(
            current=f"[{module.name}](./{module.file})",
            previous=[parent_breadcrumb.breadcrumbs]
        )
...
class IndexComponent:
    ...
    def __init__(self, modules: list[Module] = [], namespace: str = ''):
        ...
        self.breadcrumbs = Breadcrumbs(
            current=f"[openworld.sdk.{self.namespace}](./{self.file})"
        )
...
class AliasComponent:
    ...
    def __init__(self, alias: Alias, parent_breadcrumbs: Breadcrumbs):
        ...
        self.breadcrumbs = self.breadcrumbs = Breadcrumbs(
            current=f"[{self.alias.name}](./{self.file})",
            previous=[parent_breadcrumbs.breadcrumbs]
        )
    ...
    def __str__(self):
        ...
        return "\n".join([
            str(self.breadcrumbs),
            header1(f"Alias {self.alias.name}"),
            constant.NEW_LINE,
            constant.ALIAS_DOCSTRINGS,
            str(one_of_table)
        ])
